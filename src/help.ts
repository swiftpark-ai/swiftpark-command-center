...
export function buildHelpGuideMessages(channels: ChannelDefinition[] = requiredChannelDefinitions): string[] {
  ...
  const first = [
    ...
    '',
    'This server is a Discord command center for local SwiftPark agent work. Slash commands are the control surface; humans approve plans, agent work, and QA before anything moves toward merge or deploy.',
    '',
  ...
  const second = [
    ...
    '3. Use `/plan goal_id:<id> format:summary` for a short recap or `format:full` to repost the complete saved plan.',
    '4. Discuss revisions in the goal thread if present, or use `/revise-goal`.',
    '5. If something looks stuck, run `/goal-status goal_id:<id>` or `/runs`.',
    '6. `/revise-goal goal_id:<id> feedback:"Make the plan focus only on the selected spot state and mobile bottom sheet."`',
    ...
    '',
    '## Revisions',
    'Use `/revise-goal` for plan changes. Message-reading is intentionally avoided for now, so normal chat is not treated as an instruction unless a future Message Content intent workflow is explicitly enabled.',
    '',
    '## Notifications',
...
export function buildCommandDirectory(): string {
  return [
    ...
    '- `/cancel-goal goal_id:<id> reason:<optional text>`',
    '- `/cancel goal_id:<id> reason:<optional text>`',
    '- `/revise-goal goal_id:<id> feedback:<text> target:<plan|iris|atlas|sentinel|general>`',
    '- `/approve target:<plan-id|agent-id|qa-id>`',
    '- `/reject target:<id> reason:<text>`',
    '- `/run-agent goal_id:<id> agent:<orion|iris|atlas|sentinel|scout> task:<optional text>`',
