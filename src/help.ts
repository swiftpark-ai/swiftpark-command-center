...
export function buildHelpGuideMessages(channels: ChannelDefinition[] = requiredChannelDefinitions): string[] {
  ...
  const second = [
    ...
    '',
    '## Example Workflow',
    '1. `/goal description:"Improve Brighton spot map mobile clarity" primary_screen:"brighton-spot-map" agents:iris`',
    '2. Orion posts the complete plan in #orion-planning, split across messages when needed, with action buttons.',
    '3. Use `/plan format:summary` in the goal thread, or add `goal_id:<id>` outside the thread.',
    '4. Click **Ask Orion** or type normally in the goal thread for a read-only question/brainstorm reply.',
    '5. Discuss revisions in the goal thread. Plain chat stays conversational; use **Revise Plan** or `/revise-goal` when you want to save a new handoff.',
    '6. If something looks stuck, run `/goal-status` in the thread or `/runs` anywhere.',
    '7. `/revise-goal feedback:"Make the plan focus only on the selected spot state and mobile bottom sheet."`',
    '8. Click **Approve + Run**, or use `/approve` for execute-after-approval goals.',
    '9. Use **Plan Only** if you want approval without starting agents.',
    '10. Use Run Iris / Run Atlas / Run Sentinel buttons for manual overrides.',
    '11. Review screenshots in #sentinel-qa.',
    '12. `/approve target:qa-<id>` or `/reject reason:"..." target:qa-<id>`',
    '13. `/cancel reason:"No longer needed"` in the thread if the run should stop.',
    '14. Use `/clear-blocker reason:"Old Sentinel QA root error reviewed"` only when a stale failed status should stop blocking discussion.',
    '15. `/decision summary:"Ship the phone-friendly planning flow for review" rationale:"Smoke checks passed; human Discord review remains." goal_id:<id>`',
    '',
    '## Approval Rules',
